...
function generateUserInfo(user: IUser) {
  return (
    ...
    "\n" +
    "Фидбэк:     " +
    (user.feedback_mode ? "Включен" : "Выключен") +
    "\n" +
    "Админ:      " +
    (user.admin ? "Присвоен" : "Не присвоен") +
    "\n" +
    "Редактор:   " +
    (user.editor ? "Присвоен" : "Не присвоен") +
    "\n" +
    "Бан:        " +
    (user.banned ? "Забанен" : "Не забанен") +
    "\n" +
    "Язык:       " +
    (user.language || "Не выбран") +
    "\n" +
    "Клавиатура: " +
    (user.keyboard ? "Включена" : "Выключена") +
    "\n" +
    "Платформа:  " +
    (user.client || "Не выбрана") +
    "```"
  );
}
...
function generateStatistics(interval: string, stats: StatisticsData) {
  return (
    ...
    "\n" +
    "Подписанных:            " +
    stats.users.subscribed +
    "\n" +
    "Новых подп.:            " +
    stats.users.newly_subscribed +
    "\n" +
    "Отписанных:             " +
    stats.users.unsubscribed +
    "\n" +
    "Анеки\n" +
    "Всего:                  " +
    stats.aneks.count +
...
async function acceptSuggest(
  ...
  const suggest = await botApi.database.Suggest.findOneAndUpdate(
    { _id: botApi.database.Suggest.convertId(queryData[1]) },
    { approved: true },
  );
  ...
  const sendMessage = await botApi.bot.forwardMessageToChannel(suggest, {
    native: !anonymous,
  });

  await botApi.bot.sendMessage(
...
async function acceptVote(
  ...
  const approve: IApprove = await botApi.database.Approve.findById(acceptId)
    .populate("anek")
    .exec();
  ...
  if (approve) {
    ...
    approve.messages.map(async (message) => {
      const chat = await botApi.database.User.findOne({
      ...
      return botApi.bot.editMessageReplyMarkup(
        message.chat_id,
        message.message_id,
        botApi.bot.prepareApproveInlineKeyboard(
          approve.id,
          approve.anek,
          chat,
          approve.pros.length,
    ...
    return botApi.bot.answerCallbackQuery(callbackQuery.id, {
      text: "Выбор сделан",
    });
  }

  return botApi.bot.answerCallbackQuery(callbackQuery.id, {
    text: "Сообщение не найдено или уже было опубликовано",
  });
}

const shlyapaAnswers = ["как раз", "тут не гадюшник, вешать негде"];
...
function transformAneks(
  aneks: Array<IAnek & { _highlight?: { text: string[] } }>,
  user: IUser,
): InlineQueryResultArticle[] {
  return aneks.map((anek, index): InlineQueryResultArticle => {
    ...
    if (
      anek._highlight &&
      anek._highlight.text &&
      anek._highlight.text.length
    ) {
    ...
    return {
      ...
      title: translate(user.language, "anek_number", {
        number: anek.post_id || index,
      }),
      type: "article",
    };
  });
...
async function performSuggest(
  ...
  if (command[1]) {
    if (command[1] === "list") {
      ...
      await botApi.bot.sendMessage(
        message.chat.id,
        "Активные предложки на данный момент: " + suggests.length,
      );
  ...
  if (user.suggest_mode) {
    return botApi.bot.sendMessage(
      message.chat.id,
      "Вы и так уже в режиме предложки.",
    );
  }

  const suggestsLength = await botApi.database.Suggest.find({
    user: user.id,
    approved: false,
  }).count();
  ...
  return botApi.bot.sendMessage(
    message.chat.id,
    "Режим предложки включен. Вы можете писать сюда" +
...
async function performAnalysis(postId: number, message: Message) {
  ...
  if (results.ok) {
    return botApi.bot.sendMessage(
      message.chat.id,
      "Проверка анека не выявила подозрительных моментов.",
      {
        reply_to_message_id: message.message_id,
      },
    );
  ...
  return botApi.bot.sendMessage(
    message.chat.id,
    "Выявлены следующие проблемы при проверке анека: \n" +
...
botApi.bot.onCommand("debug", async (command, message, user) => {
  ...
  const sentMessage = await botApi.bot.sendMessage(
    message.from.id,
    generateDebug(),
    params,
  );
  ...
  debugTimer = setInterval(async () => {
    try {
      await botApi.bot.editMessageText(
        sentMessage.chat.id,
...
botApi.bot.onCommand("user", async (command, message, user) => {
  if (command[1] === "count") {
    ...
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "current_user_count", { count }),
    );
  }
  ...
  if (command[1] === "subscribed") {
    ...
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "current_subscribed_user_count", { count }),
    );
  }

  if (command[1] === "id") {
    if (command[2]) {
      const foundUser = await botApi.database.User.findOne({
        user_id: command[2],
      });

      return botApi.bot.sendMessage(
        message.chat.id,
        generateUserInfo(foundUser),
        { parse_mode: ParseMode.Markdown },
      );
    }
    ...
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "current_user_id", { user_id: message.from.id }),
    );
  }

  return botApi.bot.sendMessage(message.chat.id, generateUserInfo(user), {
    parse_mode: ParseMode.Markdown,
  });
});
...
botApi.bot.onCommand("anek", async (command, message, user) => {
  if (command[1] === "count") {
    ...
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "total_aneks_count", { aneks_count: count }),
    );
  } else if (command[1] && !isNaN(Number(command[1]))) {
    const foundAnek = await botApi.database.Anek.findOne()
      .skip(Number(command[1]) - 1)
      .exec();

    return botApi.bot.sendAnek(message.chat.id, foundAnek, {
...
botApi.bot.onCommand("xax", async (command, message, user) => {
  ...
  const aneksLength = await botApi.database.Anek.find({
    $where: `this.text.length <= ${max} && this.text.length >= ${min}`,
  }).count();
...
botApi.bot.onCommand("spam", async (command, message, user) => {
  ...
  await botApi.database.Anek.findOneAndUpdate(
    { post_id: command[1] },
    { spam: true },
  );
...
botApi.bot.onCommand("unspam", async (command, message, user) => {
  ...
  if (command.length <= 1) {
    return botApi.bot.sendMessage(
      message.chat.id,
      "Укажите id анека из спам листа /spam",
    );
  }
...
botApi.bot.onCommand("force_attachments", async (command, message, user) => {
  ...
  return botApi.bot.sendMessage(
    message.chat.id,
    "Автовложения " + (forceAttachments ? "включены" : "отключены" + "."),
  );
});
botApi.bot.onCommand("keyboard", async (command, message, user) => {
  if (message.chat.id !== message.from.id) {
    return botApi.bot.sendMessage(
      message.chat.id,
      "Запрещено использовать клавиатуры в группах.",
    );
  }
  ...
  if (keyboardToggle) {
    params.reply_markup = botApi.bot.prepareReplyMarkup(
      botApi.bot.prepareReplyKeyboard([[{ text: "😃" }, { text: "❌" }]], true),
    );
  } else {
    params.reply_markup = botApi.bot.prepareReplyMarkup(
      botApi.bot.prepareRemoveKeyboard(),
    );
  }

  return botApi.bot.sendMessage(
    message.chat.id,
    "Клавиатура " + (keyboardToggle ? "включена" : "отключена" + "."),
    params,
  );
});
botApi.bot.onCommand("english", async (command, message, user) => {
  ...
  return botApi.bot.sendMessage(
    message.chat.id,
    translate(user.language, "language_change"),
  );
});
...
botApi.bot.onCommand("broadcast", async (command, message, user) => {
  ...
  if (command.length <= 1) {
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "broadcast_text_missing"),
    );
  }
...
botApi.bot.onCommand("test_broadcast", async (command, message, user) => {
  ...
  if (command[1]) {
    anek = await botApi.database.Anek.findOne({ post_id: command[1] })
      .sort({ date: -1 })
      .exec();
  } else {
  ...
  const approve = new botApi.database.Approve({
    anek,
    approveTimeout: new Date(Date.now() + 30 * 1000),
  });
  const groups = await botApi.database.User.find({ approver: true });
  const messages = await botApi.bot.sendApproveAneks(
    groups,
    approve.anek,
    approve.id,
  );
...
botApi.bot.onCommand("stat", async (command, message, user) => {
  ...
  return botApi.bot.sendMessage(
    message.chat.id,
    generateStatistics(startTitle, results),
    {
      disableButtons: true,
      parse_mode: ParseMode.Markdown,
    },
...
botApi.bot.onCommand("filin", (command, message, user) =>
  botApi.bot.sendMessage(message.chat.id, translate(user.language, "filin")),
);
botApi.bot.onCommand("bret", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "Удолил"),
);
botApi.bot.onCommand("madway", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "@Lyasya кикай"),
);
botApi.bot.onCommand("do_rock", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "денис"),
);
botApi.bot.onCommand("petux", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "ti"),
);
botApi.bot.onCommand("pin", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "я не ем усы"),
);
botApi.bot.onCommand("svetlana", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "цем в лобик"),
);
botApi.bot.onCommand("bareyko", (command, message) =>
  botApi.bot.sendSticker(message.chat.id, "CAADAgADXAYAAq8ktwaLUk5_6-Z06gI"),
);
botApi.bot.onCommand("krevet", (command, message, user) =>
  botApi.bot.sendMessage(message.chat.id, translate(user.language, "krevet")),
);
botApi.bot.onCommand("shlyapa", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, generateRandomAnswer(shlyapaAnswers)),
);
botApi.bot.onCommand("gumino", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, generateRandomAnswer(guminoAnswers)),
);
botApi.bot.onCommand("detcom", (command, message) =>
  botApi.bot.sendMessage(message.chat.id, "ПОШЁЛ _НА ХУЙ_ *ХОХОЛ*", {
    parse_mode: ParseMode.Markdown,
  }),
);
botApi.bot.onCommand("forward", (command, message) =>
  botApi.bot.forwardMessage(
    message.chat.id,
    message.message_id,
    message.chat.id,
  ),
...
botApi.bot.onCommand("attach", async (command, message, user) => {
  const anek = await botApi.database.Anek.findOne({
    attachments: { $size: Number(command[1]) || 1 },
  })
    .skip(Number(command[2] || 0))
    .exec();
...
botApi.bot.onCommand("chat", (command, message) => {
  ...
  if (command[1] === "id") {
    return botApi.bot.sendMessage(
      message.chat.id,
      "id текущего чата: " + message.chat.id,
    );
  }
...
botApi.bot.onCommand("comment_list", (command, message, user) =>
  performSuggest(["/comment", "list"], message, user),
);

botApi.bot.onCommand("top_day", async (command, message, user) => {
  const count = Math.max(Math.min(Number(command[1]) || 1, 20), 1);
  const aneks = await botApi.database.Anek.find({})
    .where({
  ...
  await botApi.bot.sendMessage(
    message.chat.id,
    translate(user.language, "top_day", { count }),
  );

  return botApi.bot.fulfillAll(
    aneks.map((anek) =>
      botApi.bot.sendAnek(message.chat.id, anek, {
        forceAttachments: user.force_attachments,
      }),
    ),
  );
...
botApi.bot.onCommand("top_week", async (command, message, user) => {
  ...
  const aneks = await botApi.database.Anek.find({})
    .where({
      date: {
        $gte: Math.floor(new Date().getTime() / 1000) - 24 * 60 * 60 * 7,
      },
    })
    .sort({ likes: -1 })
    .limit(count)
  ...
  await botApi.bot.sendMessage(
    message.chat.id,
    translate(user.language, "top_week", { count }),
  );
...
botApi.bot.onCommand("top_month", async (command, message, user) => {
  ...
  const aneks = await botApi.database.Anek.find({})
    .where({
      date: {
        $gte: Math.floor(new Date().getTime() / 1000) - 24 * 60 * 60 * 30,
      },
    })
    .sort({ likes: -1 })
    .limit(count)
  ...
  await botApi.bot.sendMessage(
    message.chat.id,
    translate(user.language, "top_month", { count }),
  );
...
botApi.bot.onCommand("top_year", async (command, message, user) => {
  ...
  const aneks = await botApi.database.Anek.find({})
    .where({
      date: {
        $gte: Math.floor(new Date().getTime() / 1000) - 24 * 60 * 60 * 365,
      },
    })
    .sort({ likes: -1 })
    .limit(count)
  ...
  await botApi.bot.sendMessage(
    message.chat.id,
    translate(user.language, "top_year", { count }),
  );
...
botApi.bot.onCommand("top_ever", async (command, message, user) => {
  ...
  await botApi.bot.sendMessage(
    message.chat.id,
    translate(user.language, "top_ever", { count }),
  );
...
botApi.bot.onCommand("donate", (command, message) =>
  botApi.bot.sendInvoice(message.from.id, {
    currency: "RUB",
    description: "А то совсем нечего кушать",
    payload: "lololo",
    prices: JSON.stringify(getDonatePrices()),
...
botApi.bot.onCommand("subscribe", async (command, message) => {
  ...
  if (
    ...
    subscriber = await botApi.database.User.findOne({
  ...
  if (subscriber) {
    if (!subscriber.subscribed) {
      ...
      return botApi.bot.sendMessage(
        subscriber.user_id,
        translate(subscriber.language, "subscribe_success", {
          first_name: botApi.bot.getUserInfo(subscriber),
        }),
      );
    }
    ...
    return botApi.bot.sendMessage(
      subscriber.user_id,
      translate(subscriber.language, "subscribe_fail"),
    );
  }
});
botApi.bot.onCommand("unsubscribe", async (command, message) => {
  ...
  if (subscriber) {
    if (subscriber.subscribed) {
      ...
      return botApi.bot.sendMessage(
        subscriber.user_id,
        translate(subscriber.language, "unsubscribe_success", {
          first_name: botApi.bot.getUserInfo(subscriber),
        }),
      );
    }
    ...
    return botApi.bot.sendMessage(
      subscriber.user_id,
      translate(subscriber.language, "unsubscribe_fail"),
    );
  }
});
...
botApi.bot.onCommand("approve", async (command, message, user) => {
  ...
  if (approver) {
    if (!approver.approver) {
      ...
      return botApi.bot.sendMessage(
        approver.user_id,
        translate(approver.language, "approve_success", {
          first_name: botApi.bot.getUserInfo(approver),
        }),
      );
    }
    ...
    return botApi.bot.sendMessage(
      approver.user_id,
      translate(approver.language, "approve_fail"),
    );
  }
});
botApi.bot.onCommand("unapprove", async (command, message, user) => {
  ...
  if (approver) {
    if (approver.approver) {
      ...
      return botApi.bot.sendMessage(
        approver.user_id,
        translate(approver.language, "unapprove_success", {
          first_name: botApi.bot.getUserInfo(approver),
        }),
      );
    }
    ...
    return botApi.bot.sendMessage(
      approver.user_id,
      translate(approver.language, "unapprove_fail"),
    );
  }
});
...
botApi.bot.onCommand("approves", async (command, message, user) => {
  ...
  if (!approves.length) {
    return botApi.bot.sendMessage(
      user.user_id,
      "Анеков на рассмотрении не найдено.",
    );
  }
...
botApi.bot.onCommand("feedback", async (command, message, user) => {
  if (command[1] && user.admin) {
    ...
    return botApi.bot.sendMessage(
      Number(userId),
      "Сообщение от службы поддержки: " + command.join(" "),
    );
  }
  ...
  if (user.feedback_mode) {
    return botApi.bot.sendMessage(
      message.chat.id,
      "Вы и так уже в режиме обратной связи.",
    );
  }
  ...
  return botApi.bot.sendMessage(
    message.chat.id,
    "Режим обратной связи включен. Вы можете писать сюда" +
...
botApi.bot.onCommand("unfeedback", async (command, message, user) => {
  if (command[1] && user.admin) {
    ...
    await botApi.database.User.findOneAndUpdate(
      { user_id: userId },
      { feedback_mode: false },
    );
    ...
    return botApi.bot.sendMessage(
      message.chat.id,
      "Режим службы поддержки для пользователя " + userId + " отключен.",
    );
  }
  ...
  if (!user.feedback_mode) {
    return botApi.bot.sendMessage(
      message.chat.id,
      "Режим обратной связи и так отключен.",
    );
  }
...
botApi.bot.onCommand("grant", async (command, message, user) => {
  ...
  if (message.from.id === editorialChannel) {
    ...
    if (!newStatus) {
      return botApi.bot.sendMessage(
        message.from.id,
        `Привилегии пользователя ${botApi.bot.getUserInfo(newAdmin)} отозваны.`,
      );
    }

    return botApi.bot.sendMessage(
      message.from.id,
      `Привилегии пользователю ${botApi.bot.getUserInfo(newAdmin)} присвоены.`,
    );
  }
  ...
  await botApi.database.User.findOneAndUpdate(
    { user_id: Number(command[1]) },
    privileges,
  );
  await botApi.bot.sendMessage(
    Number(command[1]),
    "Вам были выданы привилегии администратора пользователем " +
      botApi.bot.getUserInfo(user),
  );

  return botApi.bot.sendMessage(message.chat.id, "Привилегии присвоены.");
});

botApi.bot.onCommand("birthday", (command, message) =>
  botApi.bot.sendPhoto(
    message.chat.id,
    "AgADAgADMagxGy3cYUribqypKXY_gAXZDw4ABKi3xzmLAAHaqMQjAQABAg",
  ),
);
...
botApi.bot.onCommand(
  "disable_commands",
  async (command, message: Message, user) => {
    if (!user.admin) {
      return;
    }
    ...
    if (botApi.bot.isSameChat(message)) {
      return botApi.bot.sendMessage(
        message.chat.id,
        "Нельзя отключить команды самому себе",
      );
    }
...
botApi.bot.onCommand("find", async (command, message, user) => {
  ...
  if (!searchPhrase.length) {
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "search_query_empty"),
    );
  }
  ...
  if (searchPhrase.length < 4) {
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "search_query_short"),
    );
  }
  ...
  if (!aneks.length) {
    return botApi.bot.sendMessage(
      message.chat.id,
      translate(user.language, "search_query_not_found"),
    );
  }

  return botApi.bot.sendAnek(message.chat.id, aneks[0], {
    language: user.language,
    forceAttachments: user.force_attachments,
  });
});
...
botApi.bot.on("suggest", async (suggest, user) => {
  const newSuggest = await new botApi.database.Suggest({
    ...suggest,
    user,
  }).save();
  const buttons = [
  ...
  return botApi.bot.sendMessage(user.user_id, "Предложка успешно добавлена.", {
    reply_markup: botApi.bot.prepareReplyMarkup(
      botApi.bot.prepareInlineKeyboard(buttons),
    ),
  });
});
...
botApi.bot.onCommand("schedule", async (command, message, user) => {
  ...
  if (user.scheduleCount) {
    return botApi.bot.sendMessage(user.user_id, "Рассылка уже оформлена");
  }
  ...
  return botApi.bot.sendMessage(
    userId,
    "Выберите количество присылаемых анеков",
    {
      reply_markup: botApi.bot.prepareReplyMarkup(
        botApi.bot.prepareInlineKeyboard(buttons),
      ),
...
botApi.bot.onCommand("unschedule", async (command, message, user) => {
  if (!botApi.bot.isSameChat(message)) {
    return;
  }

  if (!user.scheduleCount) {
    return botApi.bot.sendMessage(user.user_id, "Рассылка не оформлена");
  }

  await botApi.database.User.findOneAndUpdate(
    { user_id: user.user_id },
    { scheduleCount: 0, scheduleTimes: [] },
  );

  return botApi.bot.sendMessage(user.user_id, "Рассылка удалена");
});
...
botApi.bot.on("reply", async (reply, message, user) => {
  if (user.admin && reply.forward_from) {
    const replyUser = await botApi.database.User.findOne({
    ...
    return botApi.bot.sendMessage(
      ...
      "Сообщение от службы поддержки: " + message.text,
    );
  }
});

botApi.bot.onCallbackQuery(
  "s_a",
  async (args: string[], callbackQuery, user) => {
    if (!user.admin && !user.editor) {
      return;
    }
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    ...
    await botApi.bot.answerCallbackQuery(callbackQuery.id, {
      text: "Предложение одобрено анонимно",
    });
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    ...
    await botApi.database.Suggest.findOneAndRemove({
      _id: botApi.database.Suggest.convertId(args[1]),
    });
    await botApi.bot.answerCallbackQuery(callbackQuery.id, {
      text: "Предложение удалено",
    });

    return botApi.bot.deleteMessage(
      callbackQuery.from.id,
      callbackQuery.message.message_id,
    );
  },
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    ...
    await botApi.database.Suggest.findOneAndUpdate(
      { _id: botApi.database.Suggest.convertId(args[1]) },
      { public: true },
    );
    await botApi.bot.answerCallbackQuery(callbackQuery.id, {
      text: "Предложение будет опубликовано неанонимно.",
      show_alert: true,
    });
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    const params: AllMessageParams = {
      reply_to_message_id:
        callbackQuery.message && callbackQuery.message.message_id,
    };

    await botApi.bot.answerCallbackQuery(callbackQuery.id, {
      text: "Выбираю лучшие 3 переделки...",
    });

    const commentsResponse = await botApi.vk.getAllComments(Number(args[1]));
    const comments = commentsResponse
      .reduce(
        (acc: Comment[], comment: MultipleResponse<Comment>) =>
          acc.concat(comment.items),
        [],
      )
      ...
      .map((comment, index) => ({
        ...comment,
        text:
          translate(user.language, "th_place", { nth: index + 1 }) +
          comment.text,
      }));

    return botApi.bot.sendComments(callbackQuery.message.chat.id, comments, {
      ...params,
      parse_mode: ParseMode.Markdown,
      disable_web_page_preview: true,
    });
...
botApi.bot.onCallbackQuery("attach", async (args: string[], callbackQuery) => {
  ...
  return botApi.bot.sendAttachments(
    callbackQuery.message.chat.id,
    post.attachments,
    params,
  );
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    ...
    const anek = await botApi.database.Anek.findOneAndUpdate(
      { post_id: args[1] },
      { spam: true },
    );
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    ...
    await botApi.database.Anek.findOneAndUpdate(
      { post_id: args[1] },
      { spam: false, approver: user },
    );
    await botApi.bot.answerCallbackQuery(callbackQuery.id);

    return botApi.bot.sendMessage(
      callbackQuery.message.chat.id,
      "Анек помечен как нормальный.",
    );
  },
);
botApi.bot.onCallbackQuery(
...
botApi.bot.onCallbackQuery("sc_1", async (args: string[], callbackQuery) => {
  ...
  await botApi.bot.answerCallbackQuery(callbackQuery.id, {
    text: "Количество анеков выбрано",
  });

  await botApi.bot.editMessageText(
    callbackQuery.message.chat.id,
    callbackQuery.message.message_id,
    "Выберите количество рассылок в день",
    {
      reply_markup: botApi.bot.prepareReplyMarkup(
        botApi.bot.prepareInlineKeyboard(buttons),
      ),
...
botApi.bot.onCallbackQuery("sc_2", async (args: string[], callbackQuery) => {
  ...
  for (const interval of singleIntervalTable) {
    const row = buttons.addRow().addButton({
    ...
    if (times === 1) {
      row.addButton({
        callback_data: `sc_3 ${count} ${times} ${interval + 12}`,
        text: formatInterval(getIntervals(interval + 12, times)),
      });
    }
  }
  ...
  await botApi.bot.answerCallbackQuery(callbackQuery.id, {
    text: "Количество рассылок выбрано",
  });
...
botApi.bot.onCallbackQuery(
  ...
  async (args: string[], callbackQuery, user) => {
    ...
    await botApi.database.User.findOneAndUpdate(
      { user_id: user.user_id },
      { scheduleCount: count, scheduleTimes: intervalsArray },
    );
...
botApi.bot.onButton("😃", (message, user) =>
  botApi.bot.performCommand(["/anek"], message, user),
);
botApi.bot.onButton("❌", (message, user) =>
  botApi.bot.performCommand(["/keyboard"], message, user),
);
