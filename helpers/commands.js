...
module.exports = function (botApi) {
      ...
      if (command[1] === 'list') {
        ...
        return botApi.mongo.Suggest.find(query).then(function (suggests) {
          return botApi.bot.sendMessage(message.chat.id, 'Активные предложки на данный момент: ' + suggests.length).then(function () {
            return botApi.bot.forwardMessages(message.chat.id, suggests, {
  ...
  const commands = {
    ...
    '/krevet': function (command, message, user) {
      return botApi.bot.sendMessage(message.chat.id, dict.translate(user.language, 'krevet'));
    },
    '/do_rock': function (command, message) {
      return botApi.bot.sendMessage(message.chat.id, 'денис');
    },
    '/detcom': function (command, message) {
      return botApi.bot.sendMessage(message.chat.id, 'ПОШЁЛ _НА ХУЙ_ *ХОХОЛ*', {
        disableButtons: true,
        parse_mode: 'Markdown'
      });
    },
    ...
    '/birthday': function (command, message) {
      return botApi.bot.sendRequest('sendPhoto', {
          ...
          if (user) {
            if (!user.subscribed) {
              return botApi.mongo.User.update({_id: user.id}, {subscribed: true})
                .then(botApi.bot.sendMessage.bind(botApi.bot, user.user_id, dict.translate(user.language, 'subscribe_success', {first_name: user.first_name})))
    ...
    '/unsubscribe': function (command, message) {
        ...
        return new Promise(function (resolve) {
          ...
          if (user && user.subscribed) {
            return botApi.mongo.User.update({_id: user.id}, {subscribed: false})
              .then(botApi.bot.sendMessage.bind(botApi.bot, message.chat.id, dict.translate(user.language, 'unsubscribe_success')))
              .then(botApi.bot.sendMessageToAdmin.bind(botApi.bot, 'Человек отписался: ' + botApi.bot.getUserInfo(user)));
          }
          return botApi.bot.sendMessage(message.chat.id, dict.translate(user.language, 'unsubscribe_fail'));
        });
      }
      ...
      return botApi.mongo.User.findOne({user_id: message.from.id}).then(function (user) {
        if (user && user.subscribed) {
          return botApi.mongo.User.update({_id: user.id}, {subscribed: false})
            .then(botApi.bot.sendMessage.bind(botApi.bot, message.from.id, dict.translate(user.language, 'unsubscribe_success')))
            .then(botApi.bot.sendMessageToAdmin.bind(botApi.bot, 'Человек отписался: ' + botApi.bot.getUserInfo(user)));
        }
        return botApi.bot.sendMessage(message.from.id, dict.translate(user.language, 'unsubscribe_fail'));
      }).catch(function (error) {
        console.log(error);
    ...
    '/top_day': function (command, message, user) {
      ...
      return botApi.mongo.Anek
        ...
        .exec()
        .then(function (aneks) {
          return botApi.bot.sendMessages(message.chat.id, [dict.translate(user.language, 'top_daily', {count: count})].concat(aneks), {language: user.language});
        });
    },
    '/top_week': function (command, message, user) {
      ...
      return botApi.mongo.Anek
        ...
        .exec()
        .then(function (aneks) {
          return botApi.bot.sendMessages(message.chat.id, [dict.translate(user.language, 'top_weekly', {count: count})].concat(aneks), {language: user.language});
        });
    },
    '/top_month': function (command, message, user) {
      ...
      return botApi.mongo.Anek
        ...
        .exec()
        .then(function (aneks) {
          return botApi.bot.sendMessages(message.chat.id, [dict.translate(user.language, 'top_monthly', {count: count})].concat(aneks), {language: user.language});
        });
    },
    '/top_ever': function (command, message, user) {
      ...
      return botApi.mongo.Anek
        ...
        .exec()
        .then(function (aneks) {
          return botApi.bot.sendMessages(message.chat.id, [dict.translate(user.language, 'top_ever', {count: count})].concat(aneks), {language: user.language});
        });
    },
